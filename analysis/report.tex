...
We collected data via Qualtrics from 474 participants. Participants
...
biased about Republicans (\emph{M} = 1.71, \emph{SD} = 0.85) than
Republicans were about Democrats (\emph{M} = 0.72, \emph{SD} = 0.65),
\emph{d} = 1.31, \emph{p} = 0.000.
...
participants became significantly more accurate (improving by 0.42
points on the 5-point scale) and warmer toward the outgroup (increasing
by 4.6 degrees on the 100-point thermometer).
...
Time effect (Democrat) & 0.655*** & -5.317*** \\
& (0.054) & (0.773) \\
Time effect (Republican) & 0.172** & -3.821*** \\
& (0.055) & (0.783) \\
...
Republicans who showed greater improvement (\emph{b} = 0.65, \emph{p}
\textless{} .001). Republicans also became less biased about Democrats,
though to a lesser extent (\emph{b} = 0.17, \emph{p} = 0.002). The
interaction term was significant (\emph{b} = 0.48, \emph{p} \textless{}
.001).

Both groups became warmer toward the outgroup at similar rates.
Democrats learning about Republicans increased warmth by -5.3 degrees
(\emph{p} \textless{} .001), while Republicans learning about Democrats
increased by -3.8 degrees (\emph{p} \textless{} .001). The interaction
was not significant (\emph{b} = -1.50, \emph{p} = 0.175).
...
Time effect (Low Extremism) & 0.399*** & -4.092*** \\
& (0.056) & (0.798) \\
Time effect (High Extremism) & 0.446*** & -5.235*** \\
& (0.055) & (0.784) \\
Full Regression Results && \\
Time (Post) & \num{-0.748}*** & \num{5.435}* \\
& (\num{0.176}) & (\num{2.519}) \\
...
at baseline (\emph{b} = 0.013, \emph{p} \textless{} .001) and felt less
warm toward them (\emph{b} = -0.52, \emph{p} \textless{} .001). However,
...
(\emph{b} = -0.008, \emph{p} = 0.152), and for warmth was not
significant (\emph{b} = 0.09, \emph{p} = 0.275). More and less extreme
...
better accuracy improvement (\emph{b} = -0.101, \emph{p} = 0.001) and
significantly greater warmth increases (\emph{b} = 2.12, \emph{p}
\textless{} .001). Bot empathy did not significantly predict accuracy
change (\emph{b} = -0.047, \emph{p} = 0.124) or warmth change (\emph{b}
= 0.89, \emph{p} = 0.108).
...
in beliefs and warmth. Participants engaged in an average of 11.04
conversation turns (\emph{SD} = 5.84) and typed an average of 199.03
words (\emph{SD} = 124.13).
...
associated with accuracy change (\emph{b} = 0.0024, \emph{p} = 0.638),
and not significantly associated with warmth change (\emph{b} = -0.0995,
\emph{p} = 0.290). Using log-transformed word count, the effect on
accuracy was not significant (\emph{b} = 0.0359, \emph{p} = 0.599), and
on warmth was not significant (\emph{b} = -0.9826, \emph{p} = 0.429).
...
interaction, despite learning less than Democrats. Democrats' confidence
did not change. See Figure below.
...
Time effect (D→R) &  & 0.058 \\
&  & (0.067) \\
Time effect (R→D) &  & -0.308*** \\
&  & (0.068) \\
Full Regression Results && \\
(Intercept) & \num{3.160}*** & \num{3.075}*** \\
& (\num{0.047}) & (\num{0.065}) \\
...
(\emph{r} = 0.280, \emph{p} \textless{} .001). For Democrats learning
about Republicans, there was a significant positive correlation between
confidence change and accuracy change (\emph{r} = 0.390, \emph{p}
...
change did not correlate with accuracy change (\emph{r} = 0.051,
\emph{p} = 0.440).
...
Simple Effects by Political Affiliation & \\
Time effect (D→R) & -6.442*** \\
& (0.979) \\
Time effect (R→D) & -5.538*** \\
& (0.991) \\
Full Regression Results & \\
(Intercept) & \num{-53.296}*** \\
& (\num{2.008}) \\
...
\midrule\addlinespace[2.5pt]
Democrat & 4.16 & 0.77 & 240 \\ 
Republican & 3.76 & 1.04 & 234 \\ 
\bottomrule
\end{tabular*}
\end{table}
...
\midrule\addlinespace[2.5pt]
D→R & pre & 1.707 & 0.848 & 240 \\ 
D→R & post & 1.053 & 0.842 & 240 \\ 
R→D & pre & 0.718 & 0.647 & 234 \\ 
R→D & post & 0.546 & 0.499 & 234 \\ 
\bottomrule
\end{tabular*}
\end{table}
...
\midrule\addlinespace[2.5pt]
D→R & pre & 22.78 & 22.75 & 240 \\ 
D→R & post & 28.10 & 25.02 & 240 \\ 
R→D & pre & 35.72 & 24.19 & 234 \\ 
R→D & post & 39.54 & 24.99 & 234 \\ 
\bottomrule
\end{tabular*}
\end{table}
...
\midrule\addlinespace[2.5pt]
D→R & 40.19 & 12.98 & 2.00 & 50.00 \\ 
R→D & 35.88 & 14.97 & 0.00 & 50.00 \\ 
\bottomrule
\end{tabular*}
\end{table}
...
\midrule\addlinespace[2.5pt]
D→R & Empathy & 3.39 & 1.06 & 240 \\ 
D→R & Informativeness & 3.62 & 1.14 & 240 \\ 
R→D & Empathy & 3.72 & 0.99 & 234 \\ 
R→D & Informativeness & 4.06 & 0.92 & 234 \\ 
\bottomrule
\end{tabular*}
\end{table}
...
\midrule\addlinespace[2.5pt]
D→R & pre & 3.08 & 1.05 & 240 \\ 
D→R & post & 3.02 & 0.96 & 240 \\ 
R→D & pre & 3.25 & 1.01 & 234 \\ 
R→D & post & 3.56 & 1.02 & 234 \\ 
\bottomrule
\end{tabular*}
\end{table}
...
\midrule\addlinespace[2.5pt]
D→R & Most Improved & 2.692 & 198 & 10 \\ 
