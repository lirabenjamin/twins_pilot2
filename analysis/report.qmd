...
Digital twins built from survey data can accurately represent individuals' psychological characteristics [@park_generative_nodate; @toubia_database_2025], but whether marketers can extract this knowledge through conversation remains untested.
...
We recruited `r nrow(dat)` partisan participants (prescreened to exclude moderates; `r n_dr` Democrats, `r n_rd` Republicans) who conversed with a chatbot prompted to represent their political outgroup and estimated outgroup attitudes toward environmental consumption before and after the interaction.
...
At baseline, Democrats were more biased about Republicans (*M* = `r sprintf("%.2f", mean_dr)`, *SD* = `r sprintf("%.2f", sd_dr)`) than Republicans were about Democrats (*M* = `r sprintf("%.2f", mean_rd)`, *SD* = `r sprintf("%.2f", sd_rd)`), *d* = `r d`, *p* `r format_pval(p)`.
...
Of these, `r n_dr` were Democrats learning about Republicans (D→R) and `r n_rd` were Republicans learning about Democrats (R→D).
...
tbl <- modelsummary(
  ...
  stars = TRUE,
  add_rows = list(rows_party, rows_extremism_append),
  gof_map = c("nobs", "r.squared"),
  coef_omit = "Intercept",
  statistic = NULL,
  coef_map = coef_map_common,
  ...
  style_tt(i = c(1, 5, 9), bold = TRUE) %>%
  style_tt(i = c(4, 8), line = "b", line_color = "lightgray")
